...
* 	若消息发送方打算发送消息M\ :subscript:`0`\ ，则发送方首先用接收方的公钥和自己的私钥对M\ :subscript:`0`\ 进行加密、签名，得到M\ :subscript:`1`
* 	M\ :subscript:`1`\ 被投递给XMPP客户端程序，客户端将使用TLS对其进行二次加密，得到M\ :subscript:`2`
* 	M\ :subscript:`2`\ 经由服务器被发送给消息接收方的XMPP客户端程序
* 	消息接收方客户端程序收到M\ :subscript:`2`\ 后通过TLS解密还原出M\ :subscript:`1`
* 	接收方用自己的私钥和发送方的公钥对M\ :subscript:`1`\ 进行解密、签名校验，最终还原出M\ :subscript:`0`

这样做的好处在于：

由于公钥加密的介入，无论所使用的底层IM协议是否支持加密，消息本身的保密性都可得到保障。即使IM服务器尝试对消息M\ :subscript:`2`\ 进行解密，也只能还原出M\ :subscript:`1`\ ，而无法得到M\ :subscript:`0`\ ；也就是说服务器端无论如何都得不到消息明文

这样一来，即使服务器端尝试对解密出的消息“明文”进行存储，也只能存储由公钥加密保护的M\ :subscript:`1`\ ，而无法触及M\ :subscript:`0`\ ，OTR也正是由此而得名的。
